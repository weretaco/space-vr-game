...
struct Scene
{
	...
	void CreateCastle(Model* m) {
		/*
		m->AddSolidColorBox(2.2f, 0.8f, 8.0f, 4.0f, 0.7f, 7.0f, 0xff005500); // Table
		m->AddSolidColorBox(2.2f, 0.0f, 7.0f, 2.3f, 0.7f, 7.1f, 0xff005500); // Table Leg 
		m->AddSolidColorBox(2.2f, 0.7f, 8.0f, 2.3f, 0.0f, 7.9f, 0xff005500); // Table Leg 
		m->AddSolidColorBox(4.0f, 0.0f, 8.0f, 3.9f, 0.7f, 7.9f, 0xff005500); // Table Leg 
		m->AddSolidColorBox(4.0f, 0.7f, 7.0f, 3.9f, 0.0f, 7.1f, 0xff005500); // Table Leg
		*/

		// castle midpoint = (3.0, 7.5)
		m->AddSolidColorBox(1.0f, 0.7f, 6.8f, 5.0f, 0.0f, 6.2f, 0xff990000);
		m->AddSolidColorBox(1.0f, 0.7f, 8.5f, 5.0f, 0.0f, 8.2f, 0xff990000);

		m->AddSolidColorBox(1.0f, 0.7f, 8.2f, 1.3f, 0.0f, 6.8f, 0xff990000);
		m->AddSolidColorBox(4.7f, 0.7f, 8.2f, 5.0f, 0.0f, 6.8f, 0xff990000);
      
      m->AddSolidColorBox(2.2f, 3.0f, 8.0f, 3.8f, 0.0f, 7.0f, 0xff44aa00);
	}

   void CreateSpaceShip(float x, float y, float z, ShaderFill* fill) {
	   Model* m = new Model(Vector3f(0, 0, 0), fill);

      m->AddSolidColorBox(-1.0f, 1.0f+y, 1.5f+z, 1.0f, 0.0f+y, -1.5f+z, 0xff000066);
      m->AddSolidColorBox(-0.7f, 0.5f+y, 3.5f+z, 0.7f, 0.0f+y, 1.5f+z, 0xff000066);
      m->AddSolidColorBox(-0.3f, 0.8f+y, 2.3f + z, 0.3f, 0.5f+y, 1.5f + z, 0xff002200);

      m->AddSolidColorBox(-3.0f, 0.6f+y, 0.3f + z, -1.0f, 0.4f+y, -0.5f + z, 0xff000066);
      m->AddSolidColorBox(-2.15f, 0.4f+y, 0.7f + z, -1.85f, 0.2f+y, -0.9f + z, 0xff002200);

      m->AddSolidColorBox(1.0f, 0.6f+y, 0.3f + z, 3.0f, 0.4f+y, -0.5f + z, 0xff000066);
      m->AddSolidColorBox(1.85f, 0.4f+y, 0.7f + z, 2.15f, 0.2f+y, -0.9f + z, 0xff002200);

	  m->AllocateBuffers();
	  Add(m);

	  // should transform by y and z as well
	  m->Pos += Matrix4f::RotationY(0).Transform(Vector3f(x, 0, 0));

	  OutputDebugString(("Spaceship at index " + std::to_string(this->numModels - 1) + "\n").c_str());
   }
    ...
    void Init(int includeIntensiveGPUobject)
    {
		...
		//CreateCastle(m);
		CreateSpaceShip(6.5f, 0.0f, -15.0f, grid_material[3]);
		CreateSpaceShip(0.0f, 0.0f, -15.0f, grid_material[3]);
		CreateSpaceShip(-6.5f, 0.0f, -15.0f, grid_material[3]);
    }
